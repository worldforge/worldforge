...
int main(int argc, char** argv)
{
    ...
    StreamAccept accepter("simple_server", client, &bridge);

    cout << "Negotiating.... " << flush;
    // accepter.poll() does all the negotiation
    while (accepter.getState() == StreamAccept::IN_PROGRESS)
        accepter.poll();
    cout << "done." << endl;

    // Check the negotiation state to see whether it was successful
    if (accepter.getState() == StreamAccept::FAILED) {
