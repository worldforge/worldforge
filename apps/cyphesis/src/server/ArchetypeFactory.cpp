...
bool ArchetypeFactory::parseEntities(const std::map<std::string, MapType>& entitiesElement, std::map<std::string, EntityCreation>& entities) {
	for (auto& entityI: entitiesElement) {
		...
		auto entity = smart_dynamic_cast<RootEntity>(AtlasFactories::factories.createObject(entityI.second));
		if (!entity.isValid()) {
...
std::vector<Atlas::Message::Element> ArchetypeFactory::createOriginLocationThought(const LocatedEntity& entity) {
	...
	auto posProp = entity.getPropertyClassFixed<PositionProperty<LocatedEntity>>();
...
void ArchetypeFactory::addProperties(const PropertyManager<LocatedEntity>& propertyManager) {
...
void ArchetypeFactory::updateProperties(std::map<const TypeNode<LocatedEntity>*, TypeNode<LocatedEntity>::PropertiesUpdate>& changes, const PropertyManager<LocatedEntity>& propertyManager) {
	assert(m_type != nullptr);
