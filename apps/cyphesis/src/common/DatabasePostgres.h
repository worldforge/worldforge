...
struct DatabaseResultWorkerPostgres : public DatabaseResult::DatabaseResultWorker {
	...
	struct const_iterator_worker_postgres : public DatabaseResult::const_iterator_worker {
		...
		bool operator==(const const_iterator_worker& other) const
	...
	int size() const override { return PQntuples(m_res.get()); }

	int columns() const override { return PQnfields(m_res.get()); }

	bool error() const override { return (m_res == nullptr); }
