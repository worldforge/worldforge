...
std::unique_ptr<PropertyBase> TestPropertyManager::addProperty(const std::string& name) const {
	if (name == test_values<long>::name) {
		return std::make_unique<Property<long, LocatedEntity >>();
	} else if (name == test_values<double>::name) {
		return std::make_unique<Property<double, LocatedEntity>>();
	} else if (name == test_values<std::string>::name) {
		return std::make_unique<Property<std::string, LocatedEntity>>();
	} else if (name == test_values<MapType>::name) {
		return std::make_unique<Property<MapType, LocatedEntity>>();
	} else {
		return std::make_unique<SoftProperty<LocatedEntity>>();
	}
}
...
struct TestContext {
	TypeNode<LocatedEntity> m_type{"test_type"};
...
struct PropertyEntityIntegration : public Cyphesis::TestBaseWithContext<TestContext> {
	...
	void test_requirePropertyClass(const TestContext& context) {
		context.m_entity->requirePropertyClass<Property<T, LocatedEntity>>("bill");
	}

	template<class T>
	void test_requirePropertyClass_default(const TestContext& context) {
		auto& p = context.m_entity->requirePropertyClass<Property<T, LocatedEntity>>("bill",
	...
	void test_modProperty(TestContext& context) {
		...
		auto subp = dynamic_cast<Property<T, LocatedEntity>*>(p);
	...
	void test_modPropertyClass(const TestContext& context) {
		...
		auto p = context.m_entity->modPropertyClass<Property<T, LocatedEntity>>(
				test_values<T>::name
		);
		ASSERT_NOT_NULL(p);
		ASSERT_TRUE((p->flags().m_flags & prop_flag_class) == 0);
