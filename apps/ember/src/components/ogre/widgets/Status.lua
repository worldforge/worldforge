...
function StatusInstance:setEntity(entity)
	...
	if entity == nil then
		...
		self.observers = {}
		self.observers.status = Ember.AttributeObserver.new(entity, "status")
		connect(self.connectors, self.observers.status.EventChanged, self.status_Changed, self)

		self.observers.stamina = Ember.AttributeObserver.new(entity, "stamina")
		connect(self.connectors, self.observers.stamina.EventChanged, self.stamina_Changed, self)

		self.observers.strength = Ember.AttributeObserver.new(entity, "strength")
		connect(self.connectors, self.observers.strength.EventChanged, self.strength_Changed, self)

		self.connectors.deleteConnector = entity.BeingDeleted:connect(self.entity_BeingDeleted, self)
...
function Status:buildWidget(avatarEntity)
	...
	if emberOgre:getWorld():getAvatar():isAdmin() == false then
		...
		local uPosition = CEGUI.UVector2.new(CEGUI.UDim.new(0, 150), CEGUI.UDim.new(0, 0))
...
Status.createdAvatarEntityConnector = emberOgre.EventCreatedAvatarEntity:connect(function(avatarEntity)
	status = { connectors = {} }
