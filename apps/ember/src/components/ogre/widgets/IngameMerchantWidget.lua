...
function MerchantWindow.create(entity, uniqueIndex, shutdownCallback, ingameMerchantWidget)
	...
	local message = Ember.OgreView.Gui.HelpMessage.new("Merchant Window", "You have opened a merchant window, browse the wares and double click to buy items of your choice.", "merchant window", "merchantWindow")
...
function MerchantWindow:addItemForSale(itemName, price, quantityAvailable)
	...
	for i = 0, rowCount - 1 do
		if self.goods:getItemAtGridReference(CEGUI.MCLGridRef.new(i, 0)):getText() == itemName then
			existing = true
			rowNumber = i
	...
	if not existing then
		...
		self.goods:getItemAtGridReference(CEGUI.MCLGridRef.new(rowNumber, 1)):setText(price)
		self.goods:getItemAtGridReference(CEGUI.MCLGridRef.new(rowNumber, 2)):setText(quantityAvailable)
	end
end
...
IngameMerchantWidget.startConnector = emberOgre.EventCreatedAvatarEntity:connect(function()
