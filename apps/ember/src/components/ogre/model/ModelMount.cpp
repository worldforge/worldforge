...
void ModelMount::scaleNode(const WFMath::AxisBox<3>* wfBbox) {
	...
	if (!defaultOgreBoundingBox.isNull() && (defaultSize.x != 0.0f && defaultSize.y != 0.0f && defaultSize.z != 0.0f)) {
		...
		if (wfBbox && wfBbox->isValid()) {
			...
			logger->warn("Could not find any scale set in the model '{}' of type '{}'. We'll thus default to scaling the mesh so it's 0.25 meters in each dimension.",
						 getModel().getName(),
						 getModel().getDefinition()->getOrigin());
