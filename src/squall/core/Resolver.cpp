		...
		for (auto I = mPendingFetches.begin(); I != mPendingFetches.end(); I++) {
			...
			if (pending.providerResult.valid()) {
				if (pending.providerResult.get().status == ProviderResultStatus::SUCCESS) {
					...
					spdlog::debug("Successfully fetched signature {} into temporary path {}, will now store in repository.", signatureResult.signature.str_view(), pending.temporaryPath.generic_string());
