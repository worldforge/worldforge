...
{
	// modify metaserver state
	//std::map<unsigned int,std::map<std::string,std::string> > handshake_queue_;
		//boost::posix_time::milliseconds(expiry_timer_delay_milliseconds_)
	...
	out.setPacketData<unsigned int>(4,handshake);
...
MetaServer::dumpHandshake()
{
    ...
    std::cout << "hs_ptr_->size() : " << hs_ptr_->size() << std::endl;
    std::cout << "hs_ptr_->begin()->first : " << hs_ptr_->begin()->first << std::endl;
    std::cout << "handshake_queue_.size() : " << handshake_queue_.size() << std::endl;
    //std::cout << "handshake_queue_.begin() : " << handshake_queue_.begin() << std::endl;

    for( itr = hs_ptr_->begin(); itr != hs_ptr_->end(); itr++)
    {
