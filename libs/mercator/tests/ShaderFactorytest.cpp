...
int main()
{
    Mercator::ShaderFactories factories;
    ...
    GrassShader * grass_shader = dynamic_cast<GrassShader *>(shader.get());
    ...
    shader = factories.newShader("grass", params);
    grass_shader = dynamic_cast<GrassShader *>(shader.get());
    assert(grass_shader != 0);
    ...
    DepthShader * depth_shader = dynamic_cast<DepthShader *>(shader.get());
    ...
    shader = factories.newShader("depth", params);
    depth_shader = dynamic_cast<DepthShader *>(shader.get());
    assert(depth_shader != 0);
    ...
    assert(dynamic_cast<FillShader *>(shader.get()) != 0);
    ...
    LowShader * low_shader = dynamic_cast<LowShader *>(shader.get());
    assert(low_shader != 0);
    assert(low_shader->threshold() == LowShader::default_threshold);

    params[LowShader::key_threshold] = -2.f;
    shader = factories.newShader("low", params);
    low_shader = dynamic_cast<LowShader *>(shader.get());
    assert(low_shader != 0);
    ...
    BandShader * band_shader = dynamic_cast<BandShader *>(shader.get());
    ...
    shader = factories.newShader("band", params);
    band_shader = dynamic_cast<BandShader *>(shader.get());
    assert(band_shader != 0);
