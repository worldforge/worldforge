...
LongList_New(int size)
{
	...
	op = (LongListObject *) malloc(sizeof(LongListObject));
	if (op == NULL) {
		return PyErr_NoMemory();
	}
	if (size <= 0) {
...
int LongList_Size(PyObject *op)
{
	if (!LongList_Check(op)) {
		PyErr_BadInternalCall();
		return -1;
	}
	else
		return ((LongListObject *)op) -> ob_size;
}

static PyObject *indexerr;

PyObject *
LongList_GetItem(PyObject *op, int i)
{
	if (!LongList_Check(op)) {
		PyErr_BadInternalCall();
		return NULL;
	}
	if (i < 0 || i >= ((LongListObject *)op) -> ob_size) {
		if (indexerr == NULL)
	...
	return ((LongListObject *)op) -> ob_item[i];
}

int
LongList_SetItem(PyObject *op, int i, PyObject *newitem)
{
...
static int
ins1(LongListObject *self, int where, PyObject *v)
{
	int i;
	PyObject **items;
	if (v == NULL) {
...
int
LongList_Insert(PyObject *op, int where, PyObject *newitem)
{
	if (!LongList_Check(op)) {
		PyErr_BadInternalCall();
		return -1;
	...
	return ins1((LongListObject *)op, where, newitem);
}

int
LongList_Append(PyObject *op, PyObject *newitem)
{
	if (!LongList_Check(op)) {
		PyErr_BadInternalCall();
		return -1;
	...
	return ins1((LongListObject *)op,
		(int) ((LongListObject *)op)->ob_size, newitem);
}

/* Methods */

static void
LongList_dealloc(LongListObject *op)
{
	int i;
	if (op->ob_item != NULL) {
...
static int
LongList_print(LongListObject *op, FILE *fp, int flags)
{
	int i;
...
static PyObject *
LongList_repr(LongListObject *v)
{
	PyObject *s, *comma;
	int i;
...
static int
LongList_compare(LongListObject *v, LongListObject *w)
{
	int i;
...
static int
LongList_length(LongListObject *a)
{
	return a->ob_size;
}

static PyObject *
LongList_item(LongListObject *a, int i)
{
	if (i < 0 || i >= a->ob_size) {
		if (indexerr == NULL)
			indexerr = PyString_FromString(
...
static PyObject *
LongList_slice(LongListObject *a, int ilow, int ihigh)
{
	LongListObject *np;
	int i;
	if (ilow < 0)
	...
	if (ihigh < ilow)
		...
		ihigh = a->ob_size;
	np = (LongListObject *) LongList_New(ihigh - ilow);
	if (np == NULL)
		return NULL;
...
PyObject *
LongList_GetSlice(LongListObject *a, int ilow, int ihigh)
{
	if (!LongList_Check(a)) {
		PyErr_BadInternalCall();
		return NULL;
	}
	return LongList_slice((LongListObject *)a, ilow, ihigh);
}

static PyObject *
LongList_concat(LongListObject *a, PyObject *bb)
{
	int size;
	int i;
	LongListObject *np;
	if (!LongList_Check(bb)) {
		PyErr_BadArgument();
		return NULL;
	}
#define b ((LongListObject *)bb)
	size = a->ob_size + b->ob_size;
	np = (LongListObject *) LongList_New(size);
	if (np == NULL) {
		return NULL;
	}
...
static PyObject *
LongList_repeat(LongListObject *a, int n)
{
	int i, j;
	int size;
	LongListObject *np;
...
static int
LongList_ass_slice(LongListObject *a, int ilow, int ihigh, PyObject *v)
{
	...
	int k; /* Loop index */
#define b ((LongListObject *)v)
	if (v == NULL)
		n = 0;
	else if (LongList_Check(v)) {
		n = b->ob_size;
		if (a == b) {
			/* Special case "a[i:j] = a" -- copy b first */
			int ret;
			v = LongList_slice(b, 0, n);
			ret = LongList_ass_slice(a, ilow, ihigh, v);
			Py_DECREF(v);
			return ret;
		}
	}
...
int
LongList_SetSlice(LongListObject *a, int ilow, int ihigh, PyObject *v)
{
	if (!LongList_Check(a)) {
		PyErr_BadInternalCall();
		return -1;
	}
	return LongList_ass_slice((LongListObject *)a, ilow, ihigh, v);
}

static int
LongList_ass_item(LongListObject *a, int i, PyObject *v)
{
	PyObject *old_value;
	if (i < 0 || i >= a->ob_size) {
		PyErr_SetString(PyExc_IndexError,
	...
	if (v == NULL)
		return LongList_ass_slice(a, i, i+1, v);
	Py_INCREF(v);
...
static PyObject *
ins(LongListObject *self, int where, PyObject *v)
{
...
static PyObject *
LongListinsert(LongListObject *self, PyObject *args)
{
	int i;
	PyObject *v;
...
static PyObject *
LongListappend(LongListObject *self, PyObject *args)
{
	PyObject *v;
...
static PyObject *
LongListextend(LongListObject *self, PyObject *args)
{
	PyObject *b = NULL, *res = NULL;
	PyObject **items;
	int selflen = LongList_GET_SIZE(self);
	int blen;
	register int i;
	...
	if (!LongList_Check(b)) {
		PyErr_SetString(PyExc_TypeError,
				"LongList.extend() argument must be a LongList");
		return NULL;
	}
	if (LongList_GET_SIZE(b) == 0) {
		/* short circuit when b is empty */
		Py_INCREF(Py_None);
		return Py_None;
	}
	if (self == (LongListObject*)b) {
		/* as in LongList_ass_slice() we must special case the
		 * situation: a.extend(a)
		 *
		 * XXX: I think this way ought to be faster than using
		 * LongList_slice() the way LongList_ass_slice() does.
		 */
		b = LongList_New(selflen);
		if (!b)
			return NULL;
		for (i = 0; i < selflen; i++) {
			PyObject *o = LongList_GET_ITEM(self, i);
			Py_INCREF(o);
			LongList_SET_ITEM(b, i, o);
		}
	}
	else
	...
	for (i = 0; i < blen; i++) {
		PyObject *o = LongList_GET_ITEM(b, i);
		Py_INCREF(o);
		LongList_SET_ITEM(self, self->ob_size++, o);
	}
	res = Py_None;
	Py_INCREF(res);
  finally:
...
static PyObject *
LongListpop(LongListObject *self, PyObject *args)
{
	int i = -1;
	PyObject *v;
	...
	v = self->ob_item[i];
	Py_INCREF(v);
	if (LongList_ass_slice(self, i, i+1, (PyObject *)NULL) != 0) {
		Py_DECREF(v);
		return NULL;
	}
	return v;
...
static PyObject *
LongListsort(LongListObject *self, PyObject *compare)
{
	int err;

	/* self->ob_type = &immutable_LongList_type; */
	err = samplesortslice(self->ob_item,
			      self->ob_item + self->ob_size,
			      compare);
	/* self->ob_type = &LongList_Type; */
	if (err < 0)
		return NULL;
	Py_INCREF(Py_None);
	return Py_None;
...
int
LongList_Sort(PyObject *v)
{
	if (v == NULL || !LongList_Check(v)) {
		PyErr_BadInternalCall();
		return -1;
	}
	v = LongListsort((LongListObject *)v, (PyObject *)NULL);
	if (v == NULL)
		return -1;
	Py_DECREF(v);
	return 0;
...
static PyObject *
LongListreverse(LongListObject *self, PyObject *args)
{
...
int
LongList_Reverse(PyObject *v)
{
	if (v == NULL || !LongList_Check(v)) {
		PyErr_BadInternalCall();
		return -1;
	}
	v = LongListreverse((LongListObject *)v, (PyObject *)NULL);
	if (v == NULL)
		return -1;
	Py_DECREF(v);
	return 0;
...
LongList_AsTuple(PyObject* v)
{
	...
	n = ((LongListObject *)v)->ob_size;
	w = PyTuple_New(n);
	if (w == NULL)
		return NULL;
	p = ((PyTupleObject *)w)->ob_item;
	memcpy((ANY *)p,
	       (ANY *)((LongListObject *)v)->ob_item,
	       n*sizeof(PyObject *));
	while (--n >= 0) {
		Py_INCREF(*p);
		p++;
...
static PyObject *
LongListindex(LongListObject *self, PyObject *args)
{
	int i;
...
static PyObject *
LongListcount(LongListObject *self, PyObject *args)
{
	int count = 0;
	int i;
...
static PyObject *
LongListremove(LongListObject *self, PyObject *args)
{
	int i;
	...
	for (i = 0; i < self->ob_size; i++) {
		if (PyObject_Compare(self->ob_item[i], args) == 0) {
			if (LongList_ass_slice(self, i, i+1,
					   (PyObject *)NULL) != 0)
				return NULL;
			Py_INCREF(Py_None);
			return Py_None;
...
static PyMethodDef LongList_methods[] = {
	{"append",	(PyCFunction)LongListappend,	0, append_doc},
	{"insert",	(PyCFunction)LongListinsert,	0, insert_doc},
	{"extend",      (PyCFunction)LongListextend,	1, extend_doc},
	{"pop",		(PyCFunction)LongListpop,	1, pop_doc},
	{"remove",	(PyCFunction)LongListremove,	0, remove_doc},
	{"index",	(PyCFunction)LongListindex,	0, index_doc},
	{"count",	(PyCFunction)LongListcount,	0, count_doc},
	{"reverse",	(PyCFunction)LongListreverse,	0, reverse_doc},
	{"sort",	(PyCFunction)LongListsort,	0, sort_doc},
	{NULL,		NULL}		/* sentinel */
};

static PyObject *
LongList_getattr(LongListObject *f, char* name)
{
	return Py_FindMethod(LongList_methods, (PyObject *)f, name);
}

static PySequenceMethods LongList_as_sequence = {
	(inquiry)LongList_length,		/*sq_length*/
	(binaryfunc)LongList_concat,		/*sq_concat*/
	(intargfunc)LongList_repeat,		/*sq_repeat*/
	(intargfunc)LongList_item,		/*sq_item*/
	(intintargfunc)LongList_slice,		/*sq_slice*/
	(intobjargproc)LongList_ass_item,	/*sq_ass_item*/
	(intintobjargproc)LongList_ass_slice,	/*sq_ass_slice*/
};

PyTypeObject LongList_Type = {
	PyObject_HEAD_INIT(&PyType_Type)
	0,
	"LongList",
	sizeof(LongListObject),
	0,
	(destructor)LongList_dealloc,	/*tp_dealloc*/
	(printfunc)LongList_print,	/*tp_print*/
	(getattrfunc)LongList_getattr,	/*tp_getattr*/
	0,				/*tp_setattr*/
	(cmpfunc)LongList_compare, 	/*tp_compare*/
...
initLongList()
{
	...
	LongList_Type.ob_type = &PyType_Type;

	/* Create the module and add the functions */
	m = Py_InitModule("LongList", LongList_methods);

	/* Add some symbolic constants to the module */
	d = PyModule_GetDict(m);
	ErrorObject = PyErr_NewException("LongList.error", NULL, NULL);
