...
StringList_New(int size)
{
	...
	op = (StringListObject *) malloc(sizeof(StringListObject));
	if (op == NULL) {
		return PyErr_NoMemory();
	}
	if (size <= 0) {
...
int StringList_Size(PyObject *op)
{
	if (!StringList_Check(op)) {
		PyErr_BadInternalCall();
		return -1;
	}
	else
		return ((StringListObject *)op) -> ob_size;
}

static PyObject *indexerr;

PyObject *
StringList_GetItem(PyObject *op, int i)
{
	if (!StringList_Check(op)) {
		PyErr_BadInternalCall();
		return NULL;
	}
	if (i < 0 || i >= ((StringListObject *)op) -> ob_size) {
		if (indexerr == NULL)
	...
	return ((StringListObject *)op) -> ob_item[i];
}

int
StringList_SetItem(PyObject *op, int i, PyObject *newitem)
{
	...
	p = ((StringListObject *)op) -> ob_item + i;
	olditem = *p;
	*p = newitem;
	Py_XDECREF(olditem);
	return 0;
...
static int
ins1(StringListObject *self, int where, PyObject *v)
{
	int i;
	PyObject **items;
	if (v == NULL) {
...
int
StringList_Insert(PyObject *op, int where, PyObject *newitem)
{
	if (!StringList_Check(op)) {
		PyErr_BadInternalCall();
		return -1;
	...
	return ins1((StringListObject *)op, where, newitem);
}

int
StringList_Append(PyObject *op, PyObject *newitem)
{
	if (!StringList_Check(op)) {
		PyErr_BadInternalCall();
		return -1;
	...
	return ins1((StringListObject *)op,
		(int) ((StringListObject *)op)->ob_size, newitem);
}

/* Methods */

static void
StringList_dealloc(StringListObject *op)
{
	int i;
	if (op->ob_item != NULL) {
...
static int
StringList_print(StringListObject *op, FILE *fp, int flags)
{
	int i;
...
static PyObject *
StringList_repr(StringListObject *v)
{
	PyObject *s, *comma;
	int i;
...
static int
StringList_compare(StringListObject *v, StringListObject *w)
{
	int i;
...
static int
StringList_length(StringListObject *a)
{
	return a->ob_size;
}

static PyObject *
StringList_item(StringListObject *a, int i)
{
	if (i < 0 || i >= a->ob_size) {
		if (indexerr == NULL)
			indexerr = PyString_FromString(
...
static PyObject *
StringList_slice(StringListObject *a, int ilow, int ihigh)
{
	StringListObject *np;
	int i;
	if (ilow < 0)
	...
	if (ihigh < ilow)
		...
		ihigh = a->ob_size;
	np = (StringListObject *) StringList_New(ihigh - ilow);
	if (np == NULL)
		return NULL;
...
PyObject *
StringList_GetSlice(StringListObject *a, int ilow, int ihigh)
{
	if (!StringList_Check(a)) {
		PyErr_BadInternalCall();
		return NULL;
	}
	return StringList_slice((StringListObject *)a, ilow, ihigh);
}

static PyObject *
StringList_concat(StringListObject *a, PyObject *bb)
{
	int size;
	int i;
	StringListObject *np;
	if (!StringList_Check(bb)) {
		PyErr_BadArgument();
		return NULL;
	}
#define b ((StringListObject *)bb)
	size = a->ob_size + b->ob_size;
	np = (StringListObject *) StringList_New(size);
	if (np == NULL) {
		return NULL;
	}
...
static PyObject *
StringList_repeat(StringListObject *a, int n)
{
	int i, j;
	int size;
	StringListObject *np;
...
static int
StringList_ass_slice(StringListObject *a, int ilow, int ihigh, PyObject *v)
{
	...
	int k; /* Loop index */
#define b ((StringListObject *)v)
	if (v == NULL)
		n = 0;
	else if (StringList_Check(v)) {
		n = b->ob_size;
		if (a == b) {
			/* Special case "a[i:j] = a" -- copy b first */
			int ret;
			v = StringList_slice(b, 0, n);
			ret = StringList_ass_slice(a, ilow, ihigh, v);
			Py_DECREF(v);
			return ret;
		}
	}
...
int
StringList_SetSlice(StringListObject *a, int ilow, int ihigh, PyObject *v)
{
	if (!StringList_Check(a)) {
		PyErr_BadInternalCall();
		return -1;
	}
	return StringList_ass_slice((StringListObject *)a, ilow, ihigh, v);
}

static int
StringList_ass_item(StringListObject *a, int i, PyObject *v)
{
	PyObject *old_value;
	if (i < 0 || i >= a->ob_size) {
		PyErr_SetString(PyExc_IndexError,
	...
	if (v == NULL)
		return StringList_ass_slice(a, i, i+1, v);
	Py_INCREF(v);
...
static PyObject *
ins(StringListObject *self, int where, PyObject *v)
{
...
static PyObject *
StringListinsert(StringListObject *self, PyObject *args)
{
	int i;
	PyObject *v;
...
static PyObject *
StringListappend(StringListObject *self, PyObject *args)
{
	PyObject *v;
...
static PyObject *
StringListextend(StringListObject *self, PyObject *args)
{
	PyObject *b = NULL, *res = NULL;
	PyObject **items;
	int selflen = StringList_GET_SIZE(self);
	int blen;
	register int i;
	...
	if (!StringList_Check(b)) {
		PyErr_SetString(PyExc_TypeError,
				"StringList.extend() argument must be a StringList");
		return NULL;
	}
	if (StringList_GET_SIZE(b) == 0) {
		/* short circuit when b is empty */
		Py_INCREF(Py_None);
		return Py_None;
	}
	if (self == (StringListObject*)b) {
		/* as in StringList_ass_slice() we must special case the
		 * situation: a.extend(a)
		 *
		 * XXX: I think this way ought to be faster than using
		 * StringList_slice() the way StringList_ass_slice() does.
		 */
		b = StringList_New(selflen);
		if (!b)
			return NULL;
		for (i = 0; i < selflen; i++) {
			PyObject *o = StringList_GET_ITEM(self, i);
			Py_INCREF(o);
			StringList_SET_ITEM(b, i, o);
		}
	}
	else
	...
	for (i = 0; i < blen; i++) {
		PyObject *o = StringList_GET_ITEM(b, i);
		Py_INCREF(o);
		StringList_SET_ITEM(self, self->ob_size++, o);
	}
	res = Py_None;
	Py_INCREF(res);
  finally:
...
static PyObject *
StringListpop(StringListObject *self, PyObject *args)
{
	int i = -1;
	PyObject *v;
	...
	v = self->ob_item[i];
	Py_INCREF(v);
	if (StringList_ass_slice(self, i, i+1, (PyObject *)NULL) != 0) {
		Py_DECREF(v);
		return NULL;
	}
	return v;
...
static PyObject *
StringListsort(StringListObject *self, PyObject *compare)
{
	int err;

	/* self->ob_type = &immutable_StringList_type; */
	err = samplesortslice(self->ob_item,
			      self->ob_item + self->ob_size,
			      compare);
	/* self->ob_type = &StringList_Type; */
	if (err < 0)
		return NULL;
	Py_INCREF(Py_None);
	return Py_None;
...
int
StringList_Sort(PyObject *v)
{
	if (v == NULL || !StringList_Check(v)) {
		PyErr_BadInternalCall();
		return -1;
	}
	v = StringListsort((StringListObject *)v, (PyObject *)NULL);
	if (v == NULL)
		return -1;
	Py_DECREF(v);
	return 0;
...
static PyObject *
StringListreverse(StringListObject *self, PyObject *args)
{
...
int
StringList_Reverse(PyObject *v)
{
	if (v == NULL || !StringList_Check(v)) {
		PyErr_BadInternalCall();
		return -1;
	}
	v = StringListreverse((StringListObject *)v, (PyObject *)NULL);
	if (v == NULL)
		return -1;
	Py_DECREF(v);
	return 0;
...
StringList_AsTuple(PyObject* v)
{
	...
	n = ((StringListObject *)v)->ob_size;
	w = PyTuple_New(n);
	if (w == NULL)
		return NULL;
	p = ((PyTupleObject *)w)->ob_item;
	memcpy((ANY *)p,
	       (ANY *)((StringListObject *)v)->ob_item,
	       n*sizeof(PyObject *));
	while (--n >= 0) {
		Py_INCREF(*p);
		p++;
...
static PyObject *
StringListindex(StringListObject *self, PyObject *args)
{
	int i;
...
static PyObject *
StringListcount(StringListObject *self, PyObject *args)
{
	int count = 0;
	int i;
...
static PyObject *
StringListremove(StringListObject *self, PyObject *args)
{
	int i;
	...
	for (i = 0; i < self->ob_size; i++) {
		if (PyObject_Compare(self->ob_item[i], args) == 0) {
			if (StringList_ass_slice(self, i, i+1,
					   (PyObject *)NULL) != 0)
				return NULL;
			Py_INCREF(Py_None);
			return Py_None;
...
static PyMethodDef StringList_methods[] = {
	{"append",	(PyCFunction)StringListappend,	0, append_doc},
	{"insert",	(PyCFunction)StringListinsert,	0, insert_doc},
	{"extend",      (PyCFunction)StringListextend,	1, extend_doc},
	{"pop",		(PyCFunction)StringListpop,	1, pop_doc},
	{"remove",	(PyCFunction)StringListremove,	0, remove_doc},
	{"index",	(PyCFunction)StringListindex,	0, index_doc},
	{"count",	(PyCFunction)StringListcount,	0, count_doc},
	{"reverse",	(PyCFunction)StringListreverse,	0, reverse_doc},
	{"sort",	(PyCFunction)StringListsort,	0, sort_doc},
	{NULL,		NULL}		/* sentinel */
};

static PyObject *
StringList_getattr(StringListObject *f, char* name)
{
	return Py_FindMethod(StringList_methods, (PyObject *)f, name);
}

static PySequenceMethods StringList_as_sequence = {
	(inquiry)StringList_length,		/*sq_length*/
	(binaryfunc)StringList_concat,		/*sq_concat*/
	(intargfunc)StringList_repeat,		/*sq_repeat*/
	(intargfunc)StringList_item,		/*sq_item*/
	(intintargfunc)StringList_slice,		/*sq_slice*/
	(intobjargproc)StringList_ass_item,	/*sq_ass_item*/
	(intintobjargproc)StringList_ass_slice,	/*sq_ass_slice*/
};

PyTypeObject StringList_Type = {
	PyObject_HEAD_INIT(&PyType_Type)
	0,
	"StringList",
	sizeof(StringListObject),
	0,
	(destructor)StringList_dealloc,	/*tp_dealloc*/
	(printfunc)StringList_print,	/*tp_print*/
	(getattrfunc)StringList_getattr,	/*tp_getattr*/
	0,				/*tp_setattr*/
	(cmpfunc)StringList_compare, 	/*tp_compare*/
...
initStringList()
{
	...
	StringList_Type.ob_type = &PyType_Type;

	/* Create the module and add the functions */
	m = Py_InitModule("StringList", StringList_methods);

	/* Add some symbolic constants to the module */
	d = PyModule_GetDict(m);
	ErrorObject = PyErr_NewException("StringList.error", NULL, NULL);
