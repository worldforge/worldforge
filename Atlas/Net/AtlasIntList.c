...
IntList_New(int size)
{
	...
	op = (IntListObject *) malloc(sizeof(IntListObject));
	if (op == NULL) {
		return PyErr_NoMemory();
	}
	if (size <= 0) {
...
int IntList_Size(PyObject *op)
{
	if (!IntList_Check(op)) {
		...
		return ((IntListObject *)op) -> ob_size;
}

static PyObject *indexerr;

PyObject *
IntList_GetItem(PyObject *op, int i)
{
	if (!IntList_Check(op)) {
		PyErr_BadInternalCall();
		return NULL;
	}
	if (i < 0 || i >= ((IntListObject *)op) -> ob_size) {
		if (indexerr == NULL)
	...
	return ((IntListObject *)op) -> ob_item[i];
}

int
IntList_SetItem(PyObject *op, int i, PyObject *newitem)
{
...
static int
ins1(IntListObject *self, int where, PyObject *v)
{
	int i;
	PyObject **items;
	if (v == NULL) {
...
int
IntList_Insert(PyObject *op, int where, PyObject *newitem)
{
	if (!IntList_Check(op)) {
		PyErr_BadInternalCall();
		return -1;
	...
	return ins1((IntListObject *)op, where, newitem);
}

int
IntList_Append(PyObject *op, PyObject *newitem)
{
	if (!IntList_Check(op)) {
		PyErr_BadInternalCall();
		return -1;
	...
	return ins1((IntListObject *)op,
		(int) ((IntListObject *)op)->ob_size, newitem);
}

/* Methods */

static void
IntList_dealloc(IntListObject *op)
{
	int i;
	if (op->ob_item != NULL) {
...
static int
IntList_print(IntListObject *op, FILE *fp, int flags)
{
	int i;
...
static int
IntList_compare(IntListObject *v, IntListObject *w)
{
	int i;
...
static int
IntList_length(IntListObject *a)
{
	return a->ob_size;
}

static PyObject *
IntList_item(IntListObject *a, int i)
{
	if (i < 0 || i >= a->ob_size) {
		if (indexerr == NULL)
			indexerr = PyString_FromString(
...
static PyObject *
IntList_slice(IntListObject *a, int ilow, int ihigh)
{
	IntListObject *np;
	int i;
	if (ilow < 0)
	...
	if (ihigh < ilow)
		...
		ihigh = a->ob_size;
	np = (IntListObject *) IntList_New(ihigh - ilow);
	if (np == NULL)
		return NULL;
...
PyObject *
IntList_GetSlice(IntListObject *a, int ilow, int ihigh)
{
	if (!IntList_Check(a)) {
		PyErr_BadInternalCall();
		return NULL;
	}
	return IntList_slice((IntListObject *)a, ilow, ihigh);
}

static PyObject *
IntList_concat(IntListObject *a, PyObject *bb)
{
	int size;
	int i;
	IntListObject *np;
...
#define b ((IntListObject *)bb)
	size = a->ob_size + b->ob_size;
	np = (IntListObject *) IntList_New(size);
	if (np == NULL) {
		return NULL;
	}
...
static PyObject *
IntList_repeat(IntListObject *a, int n)
{
	int i, j;
	int size;
	IntListObject *np;
...
static int
IntList_ass_slice(IntListObject *a, int ilow, int ihigh, PyObject *v)
{
	...
	int k; /* Loop index */
#define b ((IntListObject *)v)
	if (v == NULL)
		n = 0;
	else if (IntList_Check(v)) {
		n = b->ob_size;
		if (a == b) {
			/* Special case "a[i:j] = a" -- copy b first */
			int ret;
			v = IntList_slice(b, 0, n);
			ret = IntList_ass_slice(a, ilow, ihigh, v);
			Py_DECREF(v);
			return ret;
		}
	}
...
int
IntList_SetSlice(IntListObject *a, int ilow, int ihigh, PyObject *v)
{
	if (!IntList_Check(a)) {
		PyErr_BadInternalCall();
		return -1;
	}
	return IntList_ass_slice((IntListObject *)a, ilow, ihigh, v);
}

static int
IntList_ass_item(IntListObject *a, int i, PyObject *v)
{
	PyObject *old_value;
	if (i < 0 || i >= a->ob_size) {
		PyErr_SetString(PyExc_IndexError,
	...
	if (v == NULL)
		return IntList_ass_slice(a, i, i+1, v);
	Py_INCREF(v);
...
static PyObject *
ins(IntListObject *self, int where, PyObject *v)
{
...
static PyObject *
IntListinsert(IntListObject *self, PyObject *args)
{
	int i;
	PyObject *v;
...
static PyObject *
IntListappend(IntListObject *self, PyObject *args)
{
	PyObject *v;
...
static PyObject *
IntListextend(IntListObject *self, PyObject *args)
{
	PyObject *b = NULL, *res = NULL;
	PyObject **items;
	int selflen = IntList_GET_SIZE(self);
	int blen;
	register int i;
	...
	if (!IntList_Check(b)) {
		PyErr_SetString(PyExc_TypeError,
				"IntList.extend() argument must be a IntList");
		return NULL;
	}
	if (IntList_GET_SIZE(b) == 0) {
		/* short circuit when b is empty */
		Py_INCREF(Py_None);
		return Py_None;
	}
	if (self == (IntListObject*)b) {
		/* as in IntList_ass_slice() we must special case the
		 * situation: a.extend(a)
		 *
		 * XXX: I think this way ought to be faster than using
		 * IntList_slice() the way IntList_ass_slice() does.
		 */
		b = IntList_New(selflen);
		if (!b)
			return NULL;
		for (i = 0; i < selflen; i++) {
			PyObject *o = IntList_GET_ITEM(self, i);
			Py_INCREF(o);
			IntList_SET_ITEM(b, i, o);
		}
	}
	else
	...
	for (i = 0; i < blen; i++) {
		PyObject *o = IntList_GET_ITEM(b, i);
		Py_INCREF(o);
		IntList_SET_ITEM(self, self->ob_size++, o);
	}
	res = Py_None;
	Py_INCREF(res);
  finally:
...
static PyObject *
IntListpop(IntListObject *self, PyObject *args)
{
	int i = -1;
	PyObject *v;
	...
	v = self->ob_item[i];
	Py_INCREF(v);
	if (IntList_ass_slice(self, i, i+1, (PyObject *)NULL) != 0) {
		Py_DECREF(v);
		return NULL;
	}
	return v;
...
static PyObject *
IntListsort(IntListObject *self, PyObject *compare)
{
	int err;

	/* self->ob_type = &immutable_IntList_type; */
	err = samplesortslice(self->ob_item,
			      self->ob_item + self->ob_size,
			      compare);
	/* self->ob_type = &IntList_Type; */
	if (err < 0)
		return NULL;
	Py_INCREF(Py_None);
	return Py_None;
...
int
IntList_Sort(PyObject *v)
{
	if (v == NULL || !IntList_Check(v)) {
		PyErr_BadInternalCall();
		return -1;
	}
	v = IntListsort((IntListObject *)v, (PyObject *)NULL);
	if (v == NULL)
		return -1;
	Py_DECREF(v);
	return 0;
...
static PyObject *
IntListreverse(IntListObject *self, PyObject *args)
{
...
int
IntList_Reverse(PyObject *v)
{
	if (v == NULL || !IntList_Check(v)) {
		PyErr_BadInternalCall();
		return -1;
	}
	v = IntListreverse((IntListObject *)v, (PyObject *)NULL);
	if (v == NULL)
		return -1;
	Py_DECREF(v);
	return 0;
...
IntList_AsTuple(PyObject* v)
{
	...
	n = ((IntListObject *)v)->ob_size;
	w = PyTuple_New(n);
	if (w == NULL)
		return NULL;
	p = ((PyTupleObject *)w)->ob_item;
	memcpy((ANY *)p,
	       (ANY *)((IntListObject *)v)->ob_item,
	       n*sizeof(PyObject *));
	while (--n >= 0) {
		Py_INCREF(*p);
		p++;
...
static PyObject *
IntListindex(IntListObject *self, PyObject *args)
{
	int i;
...
static PyObject *
IntListcount(IntListObject *self, PyObject *args)
{
	int count = 0;
	int i;
...
static PyObject *
IntListremove(IntListObject *self, PyObject *args)
{
	int i;
	...
	for (i = 0; i < self->ob_size; i++) {
		if (PyObject_Compare(self->ob_item[i], args) == 0) {
			if (IntList_ass_slice(self, i, i+1,
					   (PyObject *)NULL) != 0)
				return NULL;
			Py_INCREF(Py_None);
			return Py_None;
...
static PyMethodDef IntList_methods[] = {
	{"append",	(PyCFunction)IntListappend,	0, append_doc},
	{"insert",	(PyCFunction)IntListinsert,	0, insert_doc},
	{"extend",      (PyCFunction)IntListextend,	1, extend_doc},
	{"pop",		(PyCFunction)IntListpop,	1, pop_doc},
	{"remove",	(PyCFunction)IntListremove,	0, remove_doc},
	{"index",	(PyCFunction)IntListindex,	0, index_doc},
	{"count",	(PyCFunction)IntListcount,	0, count_doc},
	{"reverse",	(PyCFunction)IntListreverse,	0, reverse_doc},
	{"sort",	(PyCFunction)IntListsort,	0, sort_doc},
	{NULL,		NULL}		/* sentinel */
};

static PyObject *
IntList_getattr(IntListObject *f, char* name)
{
	return Py_FindMethod(IntList_methods, (PyObject *)f, name);
}

static PySequenceMethods IntList_as_sequence = {
	(inquiry)IntList_length,		/*sq_length*/
	(binaryfunc)IntList_concat,		/*sq_concat*/
	(intargfunc)IntList_repeat,		/*sq_repeat*/
	(intargfunc)IntList_item,		/*sq_item*/
	(intintargfunc)IntList_slice,		/*sq_slice*/
	(intobjargproc)IntList_ass_item,	/*sq_ass_item*/
	(intintobjargproc)IntList_ass_slice,	/*sq_ass_slice*/
};

PyTypeObject IntList_Type = {
	PyObject_HEAD_INIT(&PyType_Type)
	0,
	"IntList",
	sizeof(IntListObject),
	0,
	(destructor)IntList_dealloc,	/*tp_dealloc*/
	(printfunc)IntList_print,	/*tp_print*/
	(getattrfunc)IntList_getattr,	/*tp_getattr*/
	0,				/*tp_setattr*/
	(cmpfunc)IntList_compare, 	/*tp_compare*/
...
initIntList()
{
	...
	IntList_Type.ob_type = &PyType_Type;

	/* Create the module and add the functions */
	m = Py_InitModule("IntList", IntList_methods);

	/* Add some symbolic constants to the module */
	d = PyModule_GetDict(m);
	ErrorObject = PyErr_NewException("IntList.error", NULL, NULL);
