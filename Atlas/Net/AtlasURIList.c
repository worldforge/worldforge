...
URIList_New(int size)
{
	...
	op = (URIListObject *) malloc(sizeof(URIListObject));
	if (op == NULL) {
		return PyErr_NoMemory();
	}
	if (size <= 0) {
...
int URIList_Size(PyObject *op)
{
	if (!URIList_Check(op)) {
		...
		return ((URIListObject *)op) -> ob_size;
}

static PyObject *indexerr;

PyObject *
URIList_GetItem(PyObject *op, int i)
{
	if (!URIList_Check(op)) {
		PyErr_BadInternalCall();
		return NULL;
	}
	if (i < 0 || i >= ((URIListObject *)op) -> ob_size) {
		if (indexerr == NULL)
	...
	return ((URIListObject *)op) -> ob_item[i];
}

int
URIList_SetItem(PyObject *op, int i, PyObject *newitem)
{
...
static int
ins1(URIListObject *self, int where, PyObject *v)
{
	int i;
	PyObject **items;
	if (v == NULL) {
...
int
URIList_Insert(PyObject *op, int where, PyObject *newitem)
{
	if (!URIList_Check(op)) {
		PyErr_BadInternalCall();
		return -1;
	...
	return ins1((URIListObject *)op, where, newitem);
}

int
URIList_Append(PyObject *op, PyObject *newitem)
{
	if (!URIList_Check(op)) {
		PyErr_BadInternalCall();
		return -1;
	...
	return ins1((URIListObject *)op,
		(int) ((URIListObject *)op)->ob_size, newitem);
}

/* Methods */

static void
URIList_dealloc(URIListObject *op)
{
	int i;
	if (op->ob_item != NULL) {
...
static int
URIList_print(URIListObject *op, FILE *fp, int flags)
{
	int i;
...
static int
URIList_compare(URIListObject *v, URIListObject *w)
{
	int i;
...
static int
URIList_length(URIListObject *a)
{
	return a->ob_size;
}

static PyObject *
URIList_item(URIListObject *a, int i)
{
	if (i < 0 || i >= a->ob_size) {
		if (indexerr == NULL)
			indexerr = PyString_FromString(
...
static PyObject *
URIList_slice(URIListObject *a, int ilow, int ihigh)
{
	URIListObject *np;
	int i;
	if (ilow < 0)
	...
	if (ihigh < ilow)
		...
		ihigh = a->ob_size;
	np = (URIListObject *) URIList_New(ihigh - ilow);
	if (np == NULL)
		return NULL;
...
PyObject *
URIList_GetSlice(URIListObject *a, int ilow, int ihigh)
{
	if (!URIList_Check(a)) {
		PyErr_BadInternalCall();
		return NULL;
	}
	return URIList_slice((URIListObject *)a, ilow, ihigh);
}

static PyObject *
URIList_concat(URIListObject *a, PyObject *bb)
{
	int size;
	int i;
	URIListObject *np;
...
#define b ((URIListObject *)bb)
	size = a->ob_size + b->ob_size;
	np = (URIListObject *) URIList_New(size);
	if (np == NULL) {
		return NULL;
	}
...
static PyObject *
URIList_repeat(URIListObject *a, int n)
{
	int i, j;
	int size;
	URIListObject *np;
...
static int
URIList_ass_slice(URIListObject *a, int ilow, int ihigh, PyObject *v)
{
	...
	int k; /* Loop index */
#define b ((URIListObject *)v)
	if (v == NULL)
		n = 0;
	else if (URIList_Check(v)) {
		n = b->ob_size;
		if (a == b) {
			/* Special case "a[i:j] = a" -- copy b first */
			int ret;
			v = URIList_slice(b, 0, n);
			ret = URIList_ass_slice(a, ilow, ihigh, v);
			Py_DECREF(v);
			return ret;
		}
	}
...
int
URIList_SetSlice(URIListObject *a, int ilow, int ihigh, PyObject *v)
{
	if (!URIList_Check(a)) {
		PyErr_BadInternalCall();
		return -1;
	}
	return URIList_ass_slice((URIListObject *)a, ilow, ihigh, v);
}

static int
URIList_ass_item(URIListObject *a, int i, PyObject *v)
{
	PyObject *old_value;
	if (i < 0 || i >= a->ob_size) {
		PyErr_SetString(PyExc_IndexError,
	...
	if (v == NULL)
		return URIList_ass_slice(a, i, i+1, v);
	Py_INCREF(v);
...
static PyObject *
ins(URIListObject *self, int where, PyObject *v)
{
...
static PyObject *
URIListinsert(URIListObject *self, PyObject *args)
{
	int i;
	PyObject *v;
...
static PyObject *
URIListappend(URIListObject *self, PyObject *args)
{
	PyObject *v;
...
static PyObject *
URIListextend(URIListObject *self, PyObject *args)
{
	PyObject *b = NULL, *res = NULL;
	PyObject **items;
	int selflen = URIList_GET_SIZE(self);
	int blen;
	register int i;
	...
	if (!URIList_Check(b)) {
		PyErr_SetString(PyExc_TypeError,
				"URIList.extend() argument must be a URIList");
		return NULL;
	}
	if (URIList_GET_SIZE(b) == 0) {
		/* short circuit when b is empty */
		Py_INCREF(Py_None);
		return Py_None;
	}
	if (self == (URIListObject*)b) {
		/* as in URIList_ass_slice() we must special case the
		 * situation: a.extend(a)
		 *
		 * XXX: I think this way ought to be faster than using
		 * URIList_slice() the way URIList_ass_slice() does.
		 */
		b = URIList_New(selflen);
		if (!b)
			return NULL;
		for (i = 0; i < selflen; i++) {
			PyObject *o = URIList_GET_ITEM(self, i);
			Py_INCREF(o);
			URIList_SET_ITEM(b, i, o);
		}
	}
	else
	...
	for (i = 0; i < blen; i++) {
		PyObject *o = URIList_GET_ITEM(b, i);
		Py_INCREF(o);
		URIList_SET_ITEM(self, self->ob_size++, o);
	}
	res = Py_None;
	Py_INCREF(res);
  finally:
...
static PyObject *
URIListpop(URIListObject *self, PyObject *args)
{
	int i = -1;
	PyObject *v;
	...
	v = self->ob_item[i];
	Py_INCREF(v);
	if (URIList_ass_slice(self, i, i+1, (PyObject *)NULL) != 0) {
		Py_DECREF(v);
		return NULL;
	}
	return v;
...
static PyObject *
URIListsort(URIListObject *self, PyObject *compare)
{
	int err;

	/* self->ob_type = &immutable_URIList_type; */
	err = samplesortslice(self->ob_item,
			      self->ob_item + self->ob_size,
			      compare);
	/* self->ob_type = &URIList_Type; */
	if (err < 0)
		return NULL;
	Py_INCREF(Py_None);
	return Py_None;
...
int
URIList_Sort(PyObject *v)
{
	if (v == NULL || !URIList_Check(v)) {
		PyErr_BadInternalCall();
		return -1;
	}
	v = URIListsort((URIListObject *)v, (PyObject *)NULL);
	if (v == NULL)
		return -1;
	Py_DECREF(v);
	return 0;
...
static PyObject *
URIListreverse(URIListObject *self, PyObject *args)
{
...
int
URIList_Reverse(PyObject *v)
{
	if (v == NULL || !URIList_Check(v)) {
		PyErr_BadInternalCall();
		return -1;
	}
	v = URIListreverse((URIListObject *)v, (PyObject *)NULL);
	if (v == NULL)
		return -1;
	Py_DECREF(v);
	return 0;
...
URIList_AsTuple(PyObject* v)
{
	...
	n = ((URIListObject *)v)->ob_size;
	w = PyTuple_New(n);
	if (w == NULL)
		return NULL;
	p = ((PyTupleObject *)w)->ob_item;
	memcpy((ANY *)p,
	       (ANY *)((URIListObject *)v)->ob_item,
	       n*sizeof(PyObject *));
	while (--n >= 0) {
		Py_INCREF(*p);
		p++;
...
static PyObject *
URIListindex(URIListObject *self, PyObject *args)
{
	int i;
...
static PyObject *
URIListcount(URIListObject *self, PyObject *args)
{
	int count = 0;
	int i;
...
static PyObject *
URIListremove(URIListObject *self, PyObject *args)
{
	int i;
	...
	for (i = 0; i < self->ob_size; i++) {
		if (PyObject_Compare(self->ob_item[i], args) == 0) {
			if (URIList_ass_slice(self, i, i+1,
					   (PyObject *)NULL) != 0)
				return NULL;
			Py_INCREF(Py_None);
			return Py_None;
...
static PyMethodDef URIList_methods[] = {
	{"append",	(PyCFunction)URIListappend,	0, append_doc},
	{"insert",	(PyCFunction)URIListinsert,	0, insert_doc},
	{"extend",      (PyCFunction)URIListextend,	1, extend_doc},
	{"pop",		(PyCFunction)URIListpop,	1, pop_doc},
	{"remove",	(PyCFunction)URIListremove,	0, remove_doc},
	{"index",	(PyCFunction)URIListindex,	0, index_doc},
	{"count",	(PyCFunction)URIListcount,	0, count_doc},
	{"reverse",	(PyCFunction)URIListreverse,	0, reverse_doc},
	{"sort",	(PyCFunction)URIListsort,	0, sort_doc},
	{NULL,		NULL}		/* sentinel */
};

static PyObject *
URIList_getattr(URIListObject *f, char* name)
{
	return Py_FindMethod(URIList_methods, (PyObject *)f, name);
}

static PySequenceMethods URIList_as_sequence = {
	(inquiry)URIList_length,		/*sq_length*/
	(binaryfunc)URIList_concat,		/*sq_concat*/
	(intargfunc)URIList_repeat,		/*sq_repeat*/
	(intargfunc)URIList_item,		/*sq_item*/
	(intintargfunc)URIList_slice,		/*sq_slice*/
	(intobjargproc)URIList_ass_item,	/*sq_ass_item*/
	(intintobjargproc)URIList_ass_slice,	/*sq_ass_slice*/
};

PyTypeObject URIList_Type = {
	PyObject_HEAD_INIT(&PyType_Type)
	0,
	"URIList",
	sizeof(URIListObject),
	0,
	(destructor)URIList_dealloc,	/*tp_dealloc*/
	(printfunc)URIList_print,	/*tp_print*/
	(getattrfunc)URIList_getattr,	/*tp_getattr*/
	0,				/*tp_setattr*/
	(cmpfunc)URIList_compare, 	/*tp_compare*/
	(reprfunc)URIList_repr,		/*tp_repr*/
	0,				/*tp_as_number*/
	&URIList_as_sequence,		/*tp_as_sequence*/
	0,				/*tp_as_mapping*/
};
...
initURIList()
{
	...
	URIList_Type.ob_type = &PyType_Type;

	/* Create the module and add the functions */
	m = Py_InitModule("URIList", URIList_methods);

	/* Add some symbolic constants to the module */
	d = PyModule_GetDict(m);
	ErrorObject = PyErr_NewException("URIList.error", NULL, NULL);
