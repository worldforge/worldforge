...
class Object:
    ...
    def append(self, value):
        ...
        if (value.name=="id" and self.attr.has_key("parent")) or \
           (value.name=="parent" and self.attr.has_key("id")):
            self.append(Object(name="instance",value=[]))
    def append_inherited(self, value):
        self.attr[value.name]=value
        self.attr_list.append(value)
    def add_instance(self, obj):
        inst_obj=self.attr.get("instance")
        if not inst_obj: #checked earlier
            raise SyntaxError, ("id attribute not defined and thus instance neither",
                                self.debug)
        id=obj.attr.get("id")
    ...
    def has_parent(self, dict, id):
        ...
        for parent in self.attr['parent'].value:
            par_obj=dict[parent]
...
class ParseDef:
    ...
    def search_type(self, name):
        ...
        parent_list=obj.attr['parent'].value
        if not parent_list:
    ...
    def fill_inherited_attributes_object(self, obj, p_obj,depth):
        ...
        for parent in p_obj.attr['parent'].value:
